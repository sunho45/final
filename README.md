...
위의 그림에서 알수 있듯이 선호의 학습시간에 대한 성적값 데이터들을 $y=5x$ 가 관통하는것을 보았을 때 이 직선을 분석하면 공부시간과 성적의 상관관계를 알 수 있다. 회귀식을 추정한다는 것은 이런 상관관계를 수학적으로 나타낸다는 것을 의미한다.이런 회귀식을 통해
미래를 예측할 수 있다. 예를들어 선호가 9시간을 공부했을때 성적은 45점이 나온다고 예측할 수 있다.많은 정보가 오가는 정보화 사회속에서 미래의 결과를 예측하고 대비하는 것은 큰 장점이라고 할 수 있다. 그러므로 이런 회귀식을 그릴 수 있다는 것은 큰 장점이다.
## 선형 회귀 알고리즘의 식 분석
y|a|x|+|b
---|---|---|---|---|
종속변수(결과)|  |독립변수(원인)
...
#### T(온도)는 왜 500으로 정했나?
1000번 시행했을때 300,400대에 비해 온도가 더 빨리 0에 가까워지고 600,700대에 비해 눈에띄게 구동시간이 짧아 종합적으로 이를 고려해 평균값인 500으로 정했다. 
...
쇳물의 분자가 담금질되어 활발한 운동상태에서 식어 움직임이 둔해지는 것과 비슷하게 시간이 지날수록 온도가 낮아지는 것처럼 기존 변수를 기준으로 랜덤으로 최적해의 범위를 찾는 범위를 점차 줄이기 위해 이웃해를 위와 같이 정의해 보았다. 
...
```
d<0이라면 이웃해가 기존기울기보다 우수해이므로 기울기를 이웃해로,d>=0이라면 고정
```
...
또 하나 고려해야 하는것은 msel과 msell의 차이 d이다.d 값이 크면 p를 작게하고 d값이 작으면 p를 크게 한다. 반대로 해야 될 것 같지만 아니다. 이유는 차이가 큼에도 불구하고 p를 크게 하면 그동안 탐색한 결과가 무시되어 랜덤하게 탐색되는 결과를 낳기 때문이다. 식을 종합적으로 고려하면

![image](https://user-images.githubusercontent.com/100903674/174231176-77a7db37-9578-43ab-aa94-27ebe448b681.png)
...
```
와 같은 과정을 1000번 수행한다. 각 조건에 맞게 오차를 줄여가면서 최적의 기울기와 y절편을 찾아낸다. 
...
```
y=4.4439x+2.55616 데이터를 나타내는 회귀식이 된다.
...
선형회귀식을 조사하면서 통계학, 빅데이터 등 다양한 곳에 쓰인다는 곳을 알게 되었습니다. 활용사례도 추가하려고 했지만 다룰 것이 많고 회귀식이 무엇인지에 대한 내용이 중점이 아니기때문에  넣지 않았습니다. 비록현재는 1차 선형회귀식 밖에 못다루지만 나중에는 2차 선형회귀식을 구현해 더욱 많은 데이터를 식으로 표현해보겠습니다.
